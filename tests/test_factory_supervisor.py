...
def _fixture(tmp_path: Path):
    ...
    return plan_path, out / "factory-run.json", work, work / "factory/tasks/task-v2", provenance


def test_missing_external_adapters_quarantine_and_resume(tmp_path: Path):
    plan, run, work, task, paper = _fixture(tmp_path)
    kwargs = dict(plan_path=plan, factory_run_path=run, workdir=work, task_dir=task,
...
def test_successful_command_without_receipt_fails_closed(tmp_path: Path):
    ...
    state = factory_supervisor.run(
        plan_path=plan, factory_run_path=run, workdir=work, task_dir=task,
...
def test_rejects_weak_or_nonrectangular_calibration_contract(tmp_path: Path):
    plan, run, work, task, paper = _fixture(tmp_path)
    try:
        factory_supervisor.run(plan_path=plan, factory_run_path=run, workdir=work, task_dir=task,
            ...
            test_image="image", repetitions=4)
