...
def test_partial_nop_failure_retries_in_fresh_job_directory(tmp_path: Path):
    task = tmp_path / "task"
    task.mkdir()
    (task / "task.toml").write_text(
        '[task]\nname = "terminal-bench-science/demo-task"\n',
    ...
    marker = tmp_path / "nop-failed-once"
    harbor = tmp_path / "harbor"
    harbor.write_text(
...
job = pathlib.Path(value('--jobs-dir')) / value('--job-name')
trial = job / ('task__' + agent)
if agent == 'nop' and not pathlib.Path({str(marker)!r}).exists():
    job.mkdir(parents=True)
    pathlib.Path({str(marker)!r}).write_text('failed')
    raise SystemExit(1)
trial.joinpath('verifier').mkdir(parents=True)
trial.joinpath('artifacts').mkdir(parents=True)
reward = 1.0 if agent == 'oracle' else 0.0
passed = 2 if agent == 'oracle' else 0
failed = 0 if agent == 'oracle' else 2
job.joinpath('result.json').write_text(json.dumps({{'id': agent + '-id', 'n_total_trials': 1, 'stats': {{'n_completed_trials': 1, 'n_errored_trials': 0}}}}))
trial.joinpath('result.json').write_text(json.dumps({{'task_name': 'terminal-bench-science/demo-task', 'verifier_result': {{'rewards': {{'reward': reward}}}}, 'exception_info': None}}))
trial.joinpath('verifier/ctrf.json').write_text(json.dumps({{'results': {{'summary': {{'tests': 2, 'passed': passed, 'failed': failed, 'skipped': 0, 'pending': 0, 'other': 0}}}}}}))
trial.joinpath('verifier/reward.txt').write_text(str(reward) + '\\n')
trial.joinpath('artifacts/manifest.json').write_text(json.dumps([{{'source': '/root/submission/solver.py', 'status': 'ok' if agent == 'oracle' else 'failed'}}]))
""",
        encoding="utf-8",
    ...
    output = tmp_path / "out"
    try:
        harbor_launcher.launch_controls(task, harbor_executable=harbor, out=output)
    except harbor_launcher.HarborLauncherError as exc:
        assert "nonzero_exit" in str(exc)
    else:
        raise AssertionError("first partial Harbor failure was accepted")
    receipt = harbor_launcher.launch_controls(task, harbor_executable=harbor, out=output)
    assert receipt["tasks"][0]["control_gates"]["nop"]["gate"] == "pass"
    assert len(list((output / "jobs").glob("demo_task-oracle-attempt-*"))) == 1
    assert len(list((output / "jobs").glob("demo_task-nop-attempt-*"))) == 2
